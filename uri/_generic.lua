...
function M.authority (self, ...)
    ...
    if select('#', ...) > 0 then
        local new_auth = ...
        if new_auth then
            self.uri = scheme .. "//" .. Util.uri_encode(new_auth, "^" .. ACHAR)
        else
            self.uri = scheme
        end
...
function M.path (self, ...)
    ...
    if select('#', ...) > 0 then
        local new_path = ... or ""
        new_path = Util.uri_encode(new_path, "^" .. PCHAR)
...
function M.path_query (self, ...)
    ...
    if select('#', ...) > 0 then
        local new_path = ... or ""
        new_path = Util.uri_encode(new_path, "^" .. URI.uric)
        self.uri = before_path .. _check_path(new_path, before_path)
                   path_etc:sub(path_end + 1)
    end
