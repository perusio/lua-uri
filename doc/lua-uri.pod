...
The reverse process can be carried out with the C<unresolve> method,
...
TODO - consider removing this, and just documenting how to use C<new> for it

Returns a new URI object containing the same information as C<uri>.  You can
then modify the each one without affecting the other.  This is exactly the
same as calling C<URI:new()> and passing it the original C<uri> object.
...
character), or nil if the URI has no fragment part.  Not that an empty
fragment (zero characters long) is different from one which is completely
missing.

If C<newvalue> is supplied, changes the fragment to the new value.  If
C<newvalue> is nil then any existing fragment will be removed.
...
doesn't allow port numbers to be specified.
...
See also the section L</Relative URIs> and the C<uri:unresolve(base)> method.
...
does not allow a userinfo part (for example if it is an HTTP URI).
